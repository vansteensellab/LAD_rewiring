...
#RNAseq: data.frame, with at least columns: 
  ...
  #trim RNAseq dataframe:
  RNAseq<-RNAseq[RNAseq$chrom==chrom,]
...
#8. Function to combine domainogram, RNAseq track and gene annotation track in one nice plot:
local.RNAseq.plot<-function(RNAseq, dg, GOI, ann, mrg.left=1e6, mrg.right=1e6, logratio=FALSE, sgRNA=NA,
                                col.exp="darkred", col.ctl="navyblue", shade.exp="#FF000022", shade.ctl="#0000FF22", col.mrk="#55992F")
  #RNAseq: data.frame, with at least columns: 
  #"start" (start coordinate of RNAseq bins)
  #"stop" (stop coordinate of RNAseq bins)
  #"chrom" (chromosome)
  #"ctl" (read counts of control sample)
  #"exp" (read counts of exp sample)
...
{#determine gene and plotting coordinates:
  ...
  #trim RNAseq dataframe:
  RNAseq<-RNAseq[RNAseq$chrom==chrom,]
  RNAseq<-RNAseq[RNAseq$start>=xrange[1] & RNAseq$stop<=xrange[2],]
  ...
  #plot Expression data:
  plot(RNAseq$start, RNAseq$exp, type="n", xlim=xrange, ylim=c(-max(RNAseq$exp),max(RNAseq$exp)), xaxt="n", xlab="", cex.axis=1.5)
  rect(xleft=RNAseq$start, ybottom=0, xright=RNAseq$stop, ytop=RNAseq$exp, col=shade.exp, border=shade.exp, 
       lwd=ifelse(RNAseq$stop-RNAseq$start < diff(xrange)/100 | abs(RNAseq$exp)<0.01 ,2,0))
  rect(xleft=RNAseq$start, ybottom=0, xright=RNAseq$stop, ytop=RNAseq$ctl, col=shade.ctl, border=shade.ctl, 
       lwd=ifelse(RNAseq$stop-RNAseq$start < diff(xrange)/100 | abs(RNAseq$ctl)<0.01 ,2,0))
  abline(h=0, col="darkgrey")
  mtext(expression("log"[2]*"(RNA-seq)"), side=2, line=2.5, cex=1.3)
